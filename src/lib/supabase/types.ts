  ...
  public: {
    Tables: {
      ...
      tcle_documents: {
        ...
        Relationships: [
          {
            foreignKeyName: "tcle_documents_organization_id_fkey"
            columns: ["organization_id"]
            isOneToOne: false
            referencedRelation: "organizations"
            referencedColumns: ["id"]
          ...
          {
            foreignKeyName: "tcle_documents_paciente_id_fkey"
            columns: ["paciente_id"]
            isOneToOne: false
            referencedRelation: "pacientes"
            referencedColumns: ["id"]
          ...
          {
            foreignKeyName: "tcle_documents_profissional_id_fkey"
            columns: ["profissional_id"]
            isOneToOne: false
            referencedRelation: "users"
            referencedColumns: ["id"]
...
//   created_at: timestamp with time zone (nullable, default: now())
//   updated_at: timestamp with time zone (nullable, default: now())
//   dados_extraidos: text (nullable)
//   interpretacao: text (nullable)
//   sugestao_receita: jsonb (nullable)
...
// Table: tcle_documents
//   FOREIGN KEY tcle_documents_organization_id_fkey: FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE
//   FOREIGN KEY tcle_documents_paciente_id_fkey: FOREIGN KEY (paciente_id) REFERENCES pacientes(id) ON DELETE CASCADE
//   PRIMARY KEY tcle_documents_pkey: PRIMARY KEY (id)
//   FOREIGN KEY tcle_documents_profissional_id_fkey: FOREIGN KEY (profissional_id) REFERENCES users(id) ON DELETE SET NULL
// Table: users
...
//     USING: (organization_id = ((auth.jwt() ->> 'organization_id'::text))::uuid)
// Table: anamnese
//   Policy "Profissional pode gerenciar anamneses" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true
...
//     USING: (organization_id = get_current_user_org())
// Table: tcle_documents
//   Policy "tcle_isolation" (ALL, PERMISSIVE) roles={public}
//     USING: (organization_id = get_current_user_org())
// Table: users
...
// Table: tcle_documents
//   CREATE INDEX idx_tcle_paciente_id ON public.tcle_documents USING btree (paciente_id)
// Table: users
