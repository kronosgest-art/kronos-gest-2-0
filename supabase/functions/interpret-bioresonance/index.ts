  ...
  try {
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
         throw new Error('A análise da IA demorou muito tempo. Por favor, tente novamente em instantes.');
      }
      throw fetchError;
    } finally {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Serviço da IA indisponível no momento (Status ${response.status}). Tente novamente.`);
    }
