...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(`[interpret-bioresonance] gemini-2.5-flash failed with status ${response.status}: ${errText}`)
      console.log('[interpret-bioresonance] Falling back to model: gemini-2.5-pro')
      ...
      if (!response.ok) {
        ...
        console.error(`[interpret-bioresonance] Fallback gemini-2.5-pro also failed with status ${response.status}: ${fallbackErrText}`)
