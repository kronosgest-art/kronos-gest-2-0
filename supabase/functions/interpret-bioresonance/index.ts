...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(`[interpret-bioresonance] gemini-2.0-flash failed with status ${response.status}: ${errText}`)
      ...
      if (!response.ok) {
        ...
        console.error(`[interpret-bioresonance] Fallback gemini-1.5-pro also failed with status ${response.status}: ${fallbackErrText}`)
