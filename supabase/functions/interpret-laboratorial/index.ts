...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!examData) {
      ...
      throw { status: 400, message: "Os dados do exame (examData) não foram fornecidos na requisição.", details: "Extração dos dados estruturados do exame" }
    }
    ...
    const generateContent = async (model: string) => {
      console.log(`[interpret-laboratorial] Chamando API Gemini com modelo: ${model}`)
    ...
    try {
      console.log('[interpret-laboratorial] Tentativa com gemini-2.0-flash')
      response = await generateContent('gemini-2.0-flash')
    } catch (err: any) {
      if (err.status) throw err;
      throw { status: 500, message: err.message, details: "Tentativa de chamada ao modelo principal (gemini-2.0-flash)" }
    }
    
    if (!response.ok) {
      const errorText = await response.text()
      console.warn(`[interpret-laboratorial] Modelo principal falhou. Status ${response.status}: ${errorText}`)
      console.log('[interpret-laboratorial] Iniciando fallback para gemini-1.5-pro')
      
      try {
        response = await generateContent('gemini-1.5-pro')
      } catch (err2: any) {
        if (err2.status) throw err2;
        throw { status: 500, message: err2.message, details: "Tentativa de fallback para modelo alternativo (gemini-1.5-pro)" }
      }
