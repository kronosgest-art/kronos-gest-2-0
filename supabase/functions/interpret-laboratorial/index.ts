...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { examData, pdfBase64, pacienteId, tipoExame } = body
    const tipo = tipoExame || 'análises clínicas'
    ...
    if (!examData || String(examData).trim() === '') {
      throw {
        status: 400,
        message: 'Os dados do exame (examData) não foram fornecidos ou estão vazios.',
    ...
    if (!pacienteId) {
      throw {
        status: 400,
        message: 'O ID do paciente não foi fornecido.',
        details: 'Extração de parâmetros',
      }
    }
    ...
    if (!GEMINI_API_KEY || GEMINI_API_KEY.trim() === '') {
      throw {
        ...
        details: 'Validação de ambiente',
      }
    }

    const authHeader = req.headers.get('Authorization')
    if (!authHeader) {
      throw { status: 401, message: 'Não autorizado. Token ausente.', details: 'Autenticação' }
    }
    ...
    if (userError || !user) {
      throw { status: 401, message: 'Não autorizado. Usuário inválido.', details: 'Autenticação' }
    }
    ...
    if (!organizationId) {
      throw {
        ...
        details: 'Validação de organização',
      }
    }

    const prompt = `Você é um assistente especialista em ${tipo} e medicina integrativa.
Sua tarefa é analisar os dados extraídos do exame abaixo.
Interprete os resultados e incorpore sugestões de receita (nutracêuticos/suplementos) seguindo rigorosamente os padrões e valores de referência da SBPC/ML e da SPC, ou a literatura mais adequada para este tipo de exame.
  ...
  "sugestao_receita": [
    ...
    const generateContent = async (model: string, retries = 2) => {
      ...
      while (attempt <= retries) {
        console.log(
          `[interpret-laboratorial] Calling Gemini API with model: ${model} (attempt ${attempt + 1})`,
        )
        ...
        try {
          ...
          if (res.status === 429 && attempt < retries) {
            console.warn(
              `[interpret-laboratorial] Rate limit (429) hit on model ${model}. Retrying in 2 seconds...`,
            )
            attempt++
            await delay(2000)
            continue
          ...
          if (err.name === 'AbortError') {
            throw {
              status: 408,
              message: `A requisição para o Gemini excedeu o tempo limite de 120 segundos.`,
              details: 'Timeout',
            }
          }
          throw {
            status: 502,
            message: `Erro de rede ao conectar com o modelo: ${err.message}`,
    ...
    try {
      ...
      throw {
        status: 500,
        message: err.message,
        details: 'Tentativa de chamada ao modelo principal',
      }
    }
    ...
    if (!response.ok) {
      ...
      console.error(
        `[interpret-laboratorial] gemini-1.5-flash failed with status ${response.status}: ${errText}`,
      )
    ...
    if (!interpretacaoText) {
      console.error(
        '[interpret-laboratorial] No text returned from Gemini API.',
        JSON.stringify(data),
      )
      throw {
        status: 422,
        message:
          'A IA não retornou nenhum texto estruturado com a interpretação. Verifique os dados do exame.',
        details: 'Extração do texto gerado',
      }
    }
    ...
    try {
      ...
      console.error(
        '[interpret-laboratorial] Failed to parse JSON from AI text:',
        interpretacaoText,
      )
      throw {
        status: 500,
        message: 'A resposta da IA não pôde ser convertida para JSON.',
        details: 'Parsing do texto retornado pela IA',
      }
    }

    console.log('[interpret-laboratorial] Saving to database...')
    const { data: insertedExam, error: insertError } = await supabaseClient
      .from('exames')
      .insert({
        paciente_id: pacienteId,
        profissional_id: user.id,
        organization_id: organizationId,
        tipo_exame: tipoExame || 'laboratorial',
        data_exame: new Date().toISOString(),
        dados_extraidos: extractedText,
        interpretacao: interpretacaoJson.interpretacao_integrativa,
        sugestao_receita: interpretacaoJson.sugestao_receita,
        interpretacao_ia: JSON.stringify(interpretacaoJson),
    ...
    if (insertError) {
      console.error('[interpret-laboratorial] Error saving to database:', insertError)
    }

    console.log('[interpret-laboratorial] Finished processing successfully.')
    const finalResult = {
      ...interpretacaoJson,
      exame_id: insertedExam?.id,
    }
