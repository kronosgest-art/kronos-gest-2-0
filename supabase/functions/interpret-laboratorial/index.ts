...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { examData, pdfBase64, pacienteId } = body
...
Retorne APENAS um objeto JSON válido com a seguinte estrutura exata, sem blocos de código ou markdown em volta:
{
  ...
  "sugestao_receita": [
    ...
    if (!response.ok) {
      ...
      console.warn(
        `[interpret-laboratorial] gemini-2.5-flash failed with status ${response.status}: ${errText}`,
      )
      ...
      if (!response.ok) {
        ...
        console.error(
          `[interpret-laboratorial] Fallback gemini-2.5-pro also failed with status ${response.status}: ${fallbackErrText}`,
        )
        throw {
          status: response.status,
          message: fallbackErrText,
          details: 'Modelos principal e fallback falharam',
        }
      }
    }
