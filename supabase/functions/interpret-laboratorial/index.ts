...
import * as pdf from 'npm:pdf-parse'
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (pdfBase64) {
      console.log('[interpret-laboratorial] Extraindo texto do PDF com pdf-parse...')
      try {
        const buffer = Buffer.from(pdfBase64, 'base64')
        // @ts-ignore - The user explicitly requested to import * as pdf but use it as a function
        const data = await pdf(buffer)
        extractedText = data.text
        console.log(`[interpret-laboratorial] PDF processado com sucesso. ${extractedText.length} caracteres extraídos.`)
      } catch (err: any) {
        console.error('[interpret-laboratorial] Erro no pdf-parse:', err)
        throw { status: 422, message: `Falha ao extrair texto do PDF: ${err.message}`, details: "Extração de texto via OCR/parsing" }
      }
    } else if (examData) {
      ...
      console.error('[interpret-laboratorial] pdfBase64 or examData is missing from request body')
      throw { status: 400, message: "Os dados do exame (pdfBase64 ou examData) não foram fornecidos na requisição.", details: "Extração dos dados do exame" }
    }

    const GEMINI_API_KEY = Deno.env.get('GEMINI_API_KEY')
    if (!GEMINI_API_KEY) {
      console.error('[interpret-laboratorial] GEMINI_API_KEY is not configured')
      throw { status: 500, message: "A chave da API do Gemini (GEMINI_API_KEY) não está configurada no servidor.", details: "Validação de variáveis de ambiente do servidor" }
    }
    ...
    const generateContent = async (model: string, retries = 2) => {
      ...
      while (attempt <= retries) {
        console.log(`[interpret-laboratorial] Chamando API Gemini com modelo: ${model} (tentativa ${attempt + 1})`)
        ...
        try {
          ...
          if (res.status === 429 && attempt < retries) {
            console.warn(`[interpret-laboratorial] Rate limit (429) no modelo ${model}. Aguardando 2 segundos para tentar novamente...`)
            attempt++;
            await delay(2000);
            continue;
          }
    ...
    try {
      ...
      throw { status: 500, message: err.message, details: "Tentativa de chamada ao modelo principal (gemini-2.5-flash)" }
    }
    
    if (!response.ok) {
      const errorText = await response.text()
      console.warn(`[interpret-laboratorial] Modelo principal falhou. Status ${response.status}: ${errorText}`)
      ...
      try {
        ...
        throw { status: 500, message: err2.message, details: "Tentativa de fallback para modelo alternativo (gemini-2.5-pro)" }
      }
      
      if (!response.ok) {
        const fallbackErrText = await response.text()
        console.error(`[interpret-laboratorial] Modelo de fallback também falhou. Status ${response.status}: ${fallbackErrText}`)
        throw { status: response.status, message: fallbackErrText, details: "Chamada à API Gemini (Modelos principal e fallback falharam)" }
      }
    }
    ...
    if (!interpretacao) {
      console.error('[interpret-laboratorial] Falha: IA não retornou texto esperado. Resposta:', JSON.stringify(data))
      throw { status: 422, message: "A IA não retornou nenhum texto estruturado com a interpretação.", details: "Extração do texto da resposta gerada pela IA" }
    }
