  ...
  try {
    ...
    if (pdfBase64) {
      ...
      try {
        ...
        console.log(
          `[interpret-laboratorial] PDF processado com sucesso. ${extractedText.length} caracteres extraídos.`,
        )
      } catch (err: any) {
        console.error('[interpret-laboratorial] Erro no pdf-parse:', err)
        throw {
          status: 422,
          message: `Falha ao extrair texto do PDF: ${err.message}`,
          details: 'Extração de texto via OCR/parsing',
        }
      }
    } else if (examData) {
      ...
      throw {
        status: 400,
        message: 'Os dados do exame (pdfBase64 ou examData) não foram fornecidos na requisição.',
