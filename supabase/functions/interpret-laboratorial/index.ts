...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!pacienteId) {
      throw { status: 400, message: "O ID do paciente não foi fornecido.", details: "Extração de parâmetros" }
    }
    ...
    if (!organizationId) {
      throw { status: 400, message: "Usuário não pertence a uma organização.", details: "Validação de organização" }
    }
...
Retorne APENAS um objeto JSON válido com a seguinte estrutura exata, sem blocos de código ou markdown em volta:
{
  ...
  "sugestao_receita": [
    ...
    if (!response.ok) {
      ...
      console.error(`[interpret-laboratorial] gemini-1.5-flash failed with status ${response.status}: ${errText}`)
